...
## Project notes — kalfa.me (VERIFIED-LIVE 2026-07)

- `z.uuid()` in Zod 4 enforces RFC 9562/4122 **variant bits** — an all-ones or
  otherwise invalid-variant UUID fails validation. Tests MUST use real v4
  fixtures (e.g. from `crypto.randomUUID()`), never hand-typed `1111…` strings.
- Validation schemas live in `src/lib/validation/*` (per-domain modules,
  `result.ts` typed results). Extend the existing domain file — do not create
