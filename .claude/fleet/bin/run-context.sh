...
echo "2. בצע מה שנדרש — **בגבולות הדרגה שלך**. אם ההוראה חורגת מהם, אל תנסה"
echo "   לעקוף: זו לא תקלה, זו החסימה עושה את עבודתה."
echo "3. סגור: \`npm run fleet:agent -- complete --id <id> --summary \"<מה בוצע>\"\`."
echo "   ‏complete הוא גם התפיסה וגם ההתראה לבעלים — **אל תריץ ack על inbox**."
echo "4. לא ברור מה נדרש? \`request --role $ROLE --kind question\` עם השאלה."
echo "   נדרשת פעולה שחורגת מהרשאותיך? \`--kind approval\` עם הפקודה המדויקת."
echo "פנייה שנשארה pending בסוף ריצה = לא טיפלת בה. זה כשל."
echo
echo "**verdicts** — פניות ש**אתה** פתחת והבעלים ענה. לפני שאתה פועל על תשובה,"
echo 'חובה לקרוא `npm run fleet:agent -- ack --id <id>` ולוודא claimed:true — ack'
echo "לפני פעולה, פעם אחת בלבד. אם claimed:false — מישהו אחר כבר טיפל; אל תפעל."
echo
