...
// Request-free GA4 dashboard loader: the safe-config gate, the per-process
// cache and the mapping behind /admin/analytics — everything the admin DAL
// (src/lib/data/admin/analytics.ts) did AFTER its authorization check, moved
// here unchanged so a non-request caller (the owner WhatsApp agent's
// web_traffic_summary core) runs the SAME request, cache logic and mapper as
// the page. Within one process they also share the cache slot. The cache is
// per-process, so a caller in another process (a worker) keeps its own slot
// and can be up to one TTL (5 min) apart from the page — same definition,
// possibly a fresher GA4 read.
//
// NO authorization here: it has no session to check. Every caller must gate
// first — the admin DAL on requirePlatformStaff + view_customer_data, the owner
// agent on view_customer_data resolved server-side (plan §3.2). No DAL and no
// request-scoped Next import (the owner-agent-request-free depcruise rule
// covers this module through the core that imports it).

// Pipeline: safe-config gate → cached fetch (authorization is the caller's).
...
// Only the overview KPIs for one range: batch A, through the SAME cache key
// (`A:${range}`), request builder and mapper (mapOverview) that
// loadAnalyticsDashboard uses. In one process a page render and this call
// within the TTL share one API call; a cold call costs one batch, not four.
export async function loadAnalyticsOverview(
  range: AnalyticsRange,
): Promise<Sectioned<AnalyticsOverview>> {
  const config = await getGa4ConfigStatus();
  if (!config.ok) return notConfigured();
  const a = await fetchCached(coreSlot(`A:${range}`), CORE_TTL_MS, () =>
    fetchCoreBatch(buildCoreBatchA(range, getGa4StreamId())),
  );
  return sectioned(a, (d) => mapOverview(d.responses[0]));
}

export async function loadRealtimeSnapshot(): Promise<RealtimeResult> {
  const config = await getGa4ConfigStatus();
