...
const workflow = (token: string, type = 'trigger.webhook') => ({
  id: 'wf-1',
  eventId: null,
  definition: {
    name: 'w',
    ...
    nodes: [
      {
        ...
        type: 'node',
        position: { x: 0, y: 0 },
        data: { type, icon: 'Plugs', properties: { label: 't', description: 'd', token } },
      },
    ],
    edges: [],
  },
...
beforeEach(() => {
  vi.clearAllMocks();
  armedMock.mockResolvedValue([workflow(TOKEN)]);
...
describe('the token is the whole credential', () => {
  ...
  it('a token on a NON-webhook trigger does not open the endpoint', async () => {
    ...
    armedMock.mockResolvedValue([workflow(TOKEN, 'trigger.whatsapp_inbound')]);
