...
// The node's own matching predicates — keyword, message kind, receiving number —
// live with the rest of its contract in `nodes/trigger-whatsapp-inbound/match.ts`.
// `planRuns` below applies them; they are re-exported here for existing readers.
import { matchesKeyword, matchesKind, matchesNumber } from './nodes/trigger-whatsapp-inbound/match';

export { matchesKeyword, matchesKind, matchesNumber };
...
export function planRuns(
  ...
  for (const workflow of armed) {
    ...
    if (trigger.type !== whatsappInboundDefinition.type) continue;
