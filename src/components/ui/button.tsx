...
const buttonVariants = cva(
  // NOTE: the base sets `border` (width) but deliberately NOT a border-COLOR.
  // The exported `buttonVariants` is the raw cva fn, which CONCATENATES the base
  // and variant strings WITHOUT tailwind-merge (only the <Button> component runs
  // the result through cn()). A base `border-transparent` therefore survived
  // alongside `outline`'s `border-border` on every call site that uses
  // buttonVariants() directly — e.g. `<Link className={buttonVariants({variant:
  // 'outline'})}>` — and transparent won by CSS order, so outline links rendered
  // as bare text (VERIFIED in the browser: the guests page header). Each variant
  // now owns its border color, so no two border-color classes ever collide.
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      ...
      size: {
        default:
          "h-10 gap-1.5 px-3 has-data-[icon=inline-end]:pe-2 has-data-[icon=inline-start]:ps-2 md:h-8 md:px-2.5",
        xs: "h-6 gap-1 rounded-[min(var(--radius-md),10px)] px-2 text-xs in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pe-1.5 has-data-[icon=inline-start]:ps-1.5 [&_svg:not([class*='size-'])]:size-3",
        sm: "h-7 gap-1 rounded-[min(var(--radius-md),12px)] px-2.5 text-[0.8rem] in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pe-1.5 has-data-[icon=inline-start]:ps-1.5 [&_svg:not([class*='size-'])]:size-3.5",
        lg: "h-11 gap-1.5 px-3 has-data-[icon=inline-end]:pe-2 has-data-[icon=inline-start]:ps-2 md:h-9 md:px-2.5",
        icon: "size-8",
        "icon-xs":
