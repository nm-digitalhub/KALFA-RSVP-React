...
function EventCalendarMonthCell({
  ...
  useEventCalendar() // KALFA lint fix: binding unused upstream; kept for the context-presence assertion only
  const settings = useEventCalendarSettings()
  const viewConfig = useEventCalendarViewConfig()
...
function EventCalendarMoreIndicator({
  ...
  useEffect(
    () =>
      moreInstance.subscribe(() => {
        if (moreInstance.getState().drag !== null) setOpen(false)
      }),
    [moreInstance]
  )
