            ...
            style={
              {
                ...
                // KALFA RTL fix: logical inset (not physical `left`) so
                // overlap-packed columns follow the reading direction —
                // column 0 sits at the inline start (right edge under RTL),
                // consistent with every other start/end-based offset here.
                insetInlineStart: `${column * colPct}%`,
                width: `${span * colPct}%`,
                "--ec-z": zIndex,
              } as CSSProperties
            }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          // KALFA RTL fix: logical inset (not physical `left`). The gutter
          // and the hairline above already position with start/end logical
          // classes; under RTL the day columns flow right-to-left, so the
          // today-highlight must count columns from the inline START or it
          // paints over the wrong day.
          insetInlineStart: `calc(var(--ec-gutter-width, 4.5rem) + (100% - var(--ec-gutter-width, 4.5rem)) * ${(todayIndex * columnWidthPct) / 100})`,
