...
function useEventCalendarState<TData = unknown>(
  ...
  // KALFA fix (react-hooks/refs, real fix not a suppression): the layout
  // effect has no dependency array, so it runs after EVERY commit with THIS
  // commit's `changed` in its closure — semantically identical to upstream's
  // render-written ref flag, without touching a ref during render.
  useLayoutEffect(() => {
...
function useEventCalendarSelector<TData = unknown, TSelected = unknown>(
  ...
  // KALFA fix (react-hooks/refs, real fix not a suppression): upstream kept a
  // render-assigned `selectorRef` so a stable getSnapshot saw the latest
  // selector. useSyncExternalStore explicitly permits a NEW getSnapshot each
  // render — the current render's `selector` is simply captured in the
  // closure, and the ref (with its render-time write) disappears.
  const getSnapshot = () => {
    const next = selector(
