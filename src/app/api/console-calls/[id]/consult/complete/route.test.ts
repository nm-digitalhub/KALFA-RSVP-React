...
function req(body: unknown = {}, id = CALL_ID): Request {
  return new Request(`https://beta.kalfa.me/api/console-calls/${id}/consult/complete`, {
    method: 'POST',
...
describe('POST /api/console-calls/{id}/consult/complete', () => {
  ...
  it('400 on a smuggled field (strictObject({}))', async () => {
    mockHappy();
    const res = await POST(req({ foo: 'bar' }), ctx());
  ...
  it('completes: posts {command:"consult_complete", request_id, payload:{}}', async () => {
    mockHappy();
    const res = await POST(req(), ctx());
    expect(res.status).toBe(202);
    expect(await res.json()).toMatchObject({ completing: true });
    const [url, env] = vi.mocked(postCommandToSession).mock.calls[0];
    expect(String(url)).toBe(SECURE_URL);
    expect(env.command).toBe('consult_complete');
    expect(env.payload).toEqual({});
    expect(recordConsoleConsultAudit).toHaveBeenCalledWith(
      expect.objectContaining({ fromAgentId: USER_ID, consoleCallId: CALL_ID, phase: 'complete' }),
    );
  });
});
