...
vi.mock('@/lib/data/console-calls', async (orig) => ({
  ...
  recordConsoleConferenceAudit: vi.fn(),
...
function req(body: unknown, id = CALL_ID): Request {
  return new Request(`https://beta.kalfa.me/api/console-calls/${id}/conference`, {
    method: 'POST',
...
function mockHappy() {
  ...
  vi.mocked(recordConsoleConferenceAudit).mockResolvedValue(undefined);
}

describe('POST /api/console-calls/{id}/conference', () => {
  ...
  it('409 when the target is unresolvable', async () => {
    mockHappy();
    vi.mocked(resolveTransferTarget).mockResolvedValue({ ok: false, reason: 'self' });
    const res = await POST(req({ to_agent_id: TARGET_ID }), ctx());
    expect(res.status).toBe(409);
  ...
  it('adds to conference: posts {command:"conference_add", request_id, payload.vox_username}', async () => {
    mockHappy();
    const res = await POST(req({ to_agent_id: TARGET_ID }), ctx());
    expect(res.status).toBe(202);
    expect(await res.json()).toMatchObject({ conferencing: true, to_agent_id: TARGET_ID });
