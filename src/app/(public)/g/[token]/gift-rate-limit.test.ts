...
// Tripwire: the public gift page's rate-limit key must be built from a
...
describe('public gift page rate-limit key never embeds the raw token', () => {
  ...
  it('does not build the rateLimit key from the raw token', () => {
    expect(source).not.toMatch(/rateLimit\(`gift:view:\$\{token\}/);
  });

  it('builds the rate-limit key from the fingerprint variable', () => {
    expect(source).toMatch(/rateLimit\(`gift:view:\$\{fp\}/);
  });
});
