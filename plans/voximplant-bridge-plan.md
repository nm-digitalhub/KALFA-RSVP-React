...
11. **ביקורת Branch B (מיידי, לפני redeploy)**: `WebSocketParameters.trace` מתועד כ־dump מלא של זרם ה־WS (כולל טוקנים/PII) לקובץ plaintext ב־S3 — לוודא שהוא **לא** דלוק בתרחיש הפרוס, ולקבע `privacy:true` על ה־WS של גשר ה־Groq (ה־payload נושא bearer). בנוסף: `TTSEffectsProfile.TelephonyClassApplication` (Google בלבד, post-processing מכוון־טלפוניה) — מנוף זול לשיפור איכות שמע עברית שטרם נוסה, מועמד ל־A/B; אין ASRDictionary עברי (ביאוס אוצר־מילים נשאר בשכבת ה־LLM); כשל `TTSOptions.pitch` צף אסינכרונית כ־`PlaybackFinished` עם error 400, לא כ־throw.
