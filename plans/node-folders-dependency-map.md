...
> עדכון 24.9.2026: סעיף 8א במסמך הזה הוא הצעת הביניים המקורית. המבנה המחייב להמשך המיפוי הוא `plans/node-folders-file-matrix.md`: שמות תיקייה category-prefixed, שישה קבצי בסיס לכל צומת (`definition.ts`, ארבעת קבצי הספק ו-`runtime.ts`), בלי `index.ts` בתיקיית צומת, ו-`definition.ts`/ `runtime.ts` נקיים מ-`@workflowbuilder/sdk`.

> עדכון 24.9.2026, 08:41: ההעברה הושלמה (`f99836d`..`fd03b07`, כל 23 הצמתים). המסמך הזה מתאר את הקוד ב-`72ab238`, ומספרי השורות בו מתייחסים לגרסה ההיא. הקוד זז מאז, ומספרי השורות לא עודכנו. היכן שטענה כאן כבר לא נכונה, נוספה הערה קצרה. המצב הסופי, והסטיות מהתוכנית, מופיעים ב-`plans/node-folders-file-matrix.md` §8.
...
6. **[נמדד]** כלל השמירה בודק רק את הנתיב `catalogue/schemas.ts` (`.dependency-cruiser.cjs:25-26`). הוא לא יראה קובצי מבנה חדשים בתיקיות. אין כלל שאוסר את ספריית העורך עצמה בשרת, ו־`worker/` לא כלול ב־`from`. *(עדכון: נוסף הכלל `server-code-must-not-reach-the-editor-sdk` ב־`f99836d`, והוא כולל את `worker/`. ראו המטריצה §8.)*
...
*(עדכון, ראו המטריצה §8: `KalfaNodeConfig` כולל עכשיו 23 מתוך 23, ובדיקת קומפילציה שומרת עליו (`3c814eb`). `NODE_ACTIVITY_PROFILES` נבנה מההגדרות, ו־`node-definitions.test.ts` דורש מכל הגדרה תקציב מפורש, אובייקט או `'default'`. `GUEST_SCOPED_NODE_TYPES` נבדק מול הדגל `guestScoped` של כל הגדרה, והסריקה קוראת את ה־runtimes שבתיקיות.)*
...
*(עדכון: סריקות המקור נכתבו מחדש ב־`f99836d` וב־`a379a17` כך שיקראו את כל קובצי הקוד בתיקיות הצמתים, דרך `node-sources.ts`. ראו המטריצה §8.)*
...
**התבניות מחברות סוגים לפי מחרוזות** של שמות פלט (TM:798, 815, 990, 1012, 1183-1219, 2001, 2098-2106). **[נמדד]** לכן `templates.ts` צריך להישאר מחוץ לתיקיות הצמתים. **[מוסק]** *(עדכון: `templates.ts` פוצל ב־`499e191` לקובץ לכל תבנית, תחת `catalogue/templates/`. התיקייה עדיין מחוץ לתיקיות הצמתים.)*
...
4. **התיקון (24.9):**
   ...
   - *(עדכון: הקובץ `catalogue/sumit-card-output.ts` נמחק ב־`143eae4`, ותוכנו הוטמע ב־`nodes/trigger-sumit-card/definition.ts`, שממנו קוראים עכשיו שלושת הקבצים. ראו המטריצה §8, "סטיות מהתוכנית".)*
5. **האימות:**
   - `worker:deps` נקי.
...
3. **`KalfaNodeConfig` חסר את `start_voice_call`,** ואין בדיקה שתופסת את זה. *(עדכון: תוקן ב־`3c814eb`, עם `StartVoiceCallConfig` ובדיקת קומפילציה.)*
4. **שני טיפוסי המטען של הטריגר התפצלו.** `TriggerPayload` חסר את `inboxRowId` ואת `query`.
5. **ל־`trigger.webhook` אין `allOf`,** למרות ההערה. הכלל נאכף רק בזמן חימוש. *(עדכון: ההערה תוקנה ב־`86c6d19`, ו־`schema.ts` כבר לא טוען ל־`allOf`. ה־`allOf` עדיין חסר, והכלל עדיין נאכף רק בזמן חימוש.)*
...
**נשאר במקום:** `templates.ts` וכלל החימוש של אורח ב־`arm-check.ts`. *סיבה:* שניהם מחברים כמה צמתים (§5). *(עדכון: התבניות פוצלו ל־`catalogue/templates/` ב־`499e191`, ועדיין מחוץ לתיקיות הצמתים.)*

## 8א. מבנה התיקיות בסוף, הצעה **[מוסק]**

> עדכון: ההצעה הזאת לא יושמה כמו שהיא. המבנה בפועל, לפי המטריצה, הוא:
> - שמות תיקייה category-prefixed kebab-case, למשל `trigger-whatsapp-inbound` ולא `trigger.whatsapp_inbound`.
> - שישה קבצים שטוחים בכל תיקייה, בלי `editor/`, `shared.ts` או `server.ts`.
> - פונקציות ה-schema הדינמיות נמצאות ב-`schema.ts`. `triggerSchemaFor` נקרא עכשיו `whatsappInboundSchemaFor`.
> - `match.ts` קיים רק ב-`trigger-whatsapp-inbound` וב-`trigger-webhook`.
> - `output-fields.ts`, `sample-output.ts` ו-`outcome.ts` לא נוצרו.
> - `templates.ts` פוצל ל-`catalogue/templates/`.
>
> הפירוט והסיבות מופיעים במטריצה §8, "סטיות מהתוכנית".
