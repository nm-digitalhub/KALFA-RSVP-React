#!/usr/bin/env node
// Controlled deploy of the KALFA signup-confirmation email template to the
// linked Supabase HOSTED project via the Management API.
//
// Sibling of deploy-recovery-email-template.mjs — same safety model, same
// /auth/confirm token_hash contract, different mailer fields. The default
// Supabase template for this mail is bare English; a Hebrew-first product must
// not send it.
//
...
//
// Usage:
//   SUPABASE_ACCESS_TOKEN=<pat> node scripts/deploy-confirmation-email-template.mjs         # dry-run
//   SUPABASE_ACCESS_TOKEN=<pat> node scripts/deploy-confirmation-email-template.mjs --apply # apply
...
const TEMPLATE_PATH = join(ROOT, 'supabase/templates/confirmation.html');
const SUBJECT = 'אישור כתובת המייל שלך ב־KALFA';
...
const EXPECTED_HREF =
  '{{ .RedirectTo }}?token_hash={{ .TokenHash }}&type=signup&next=/app';
...
if (hrefDecoded !== EXPECTED_HREF) {
  fail(
    'Confirmation link href does not match the exact contract.' +
...
const curSubject = current.mailer_subjects_confirmation ?? '(default)';
const curContent = current.mailer_templates_confirmation_content ?? '(default — {{ .ConfirmationURL }})';

console.log('--- current remote confirmation template ---');
...
console.log(`\nWill PATCH ONLY: mailer_subjects_confirmation, mailer_templates_confirmation_content`);
console.log(`(site_url stays untouched: ${current.site_url})\n`);

// The confirmation link's {{ .RedirectTo }} is <APP_ORIGIN>/auth/confirm. If that URL
...
console.log(`confirm-link target : ${confirmUrl ?? '(APP_ORIGIN not set in env)'}`);
...
if (after.mailer_templates_confirmation_content !== html) {
  fail('Deployed recovery content does not match the file — the PATCH did not land as sent.');
}
if (after.mailer_subjects_confirmation !== SUBJECT) {
  fail('Deployed recovery subject does not match the file — the PATCH did not land as sent.');
}
console.log('✓ Confirmation template deployed AND verified: content + subject landed; site_url + uri_allow_list intact.');
