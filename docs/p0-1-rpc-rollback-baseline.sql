-- P0-1 (A2) — ROLLBACK BASELINE for try_record_billed_result.
--
-- This is the VERBATIM ORIGINAL live body (identical to
-- 20260630223635_event_lifecycle_state_model.sql:196-246, captured from the live DB via
-- pg_get_functiondef on 2026-07-12, BEFORE the exposure-gate change in
-- 20260712122500_billed_result_exposure_gate.sql).
--
-- To roll back the A2 change, run this create-or-replace as-is. It restores the legacy
-- 'not_authorized' set-membership gate and drops the app_settings.billing_exposure_gate
-- toggle path from the function. (The billing_exposure_gate column may remain; it is inert
-- once this baseline is restored.) NOT a migration — kept here so rollback is copy-paste.
