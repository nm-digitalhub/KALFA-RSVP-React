...
1. Type `{{`. A suggestions panel opens, grouped by upstream node, with each property’s label and description.

1. Pick a suggestion. The field shows a chip labeled `{{ <Node Label> · <Property> }}` (for example `{{ Classify · Response }}`). Continue typing around the chip to mix references with literal text. Press `Esc` or click outside to dismiss the panel.
...
1. Configure `Classify` with a prompt that produces a category, e.g. *“Classify the following request as one of: refund, support, sales.”*

1. Open `Respond`. Focus the **System prompt** field and type `{{`.

1. From the picker, choose **Classify · Response**. The field renders a chip labeled `{{ Classify · Response }}`; the diagram stores it as `{{nodes.<classify-id>.response}}`.

1. Type the rest of the prompt around the chip. The editor view looks like this:
```plaintext
...
1. Add a branch labeled **High priority**, with one condition.

1. In the condition’s **X** field, type the reference manually (the picker does not surface `trigger.<path>` today):
```plaintext
{{trigger.priority}}
```
1. Set the comparison operator to `is equal`.

1. Set the **Y** field to the literal `high`.
...
- [Add a custom node](https://www.workflowbuilder.io/docs/guides/add-a-custom-node/) - register a new node type that participates in the picker

- [AI Agent node](https://www.workflowbuilder.io/docs/nodes/ai-agent/) - uses the picker on the system prompt

- [Decision node](https://www.workflowbuilder.io/docs/nodes/decision/) - uses the picker on condition operands

[Previous Build a plugin](https://www.workflowbuilder.io/docs/guides/build-a-plugin/)
[Next Save Diagrams to Database](https://www.workflowbuilder.io/docs/guides/save-diagrams-to-database/)
