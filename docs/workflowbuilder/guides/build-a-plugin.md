...
Combined with [Custom JsonForms control](https://www.workflowbuilder.io/docs/guides/custom-jsonforms-control/), the three registration functions below cover most customisation needs.
...
```
Plugins can also be called directly — the registries are currently module-global singletons. See [Known limitations](https://www.workflowbuilder.io/docs/get-started/side-effects/#known-limitations).
...
Pass the slot’s props type as the type parameter so `modifyProps` is checked against the actual host’s prop shape. Slots that target a built-in component export a matching `*Props` type from the SDK barrel — for example, `DiagramContainerProps` for the `'DiagramContainer'` slot, [`ProjectSelectionProps`](https://www.workflowbuilder.io/docs/api/components/projectselectionprops/) for `'ProjectSelection'`, and [`PropertiesBarProps`](https://www.workflowbuilder.io/docs/api/components/propertiesbarprops/) for `'PropertiesBar'`.
```tsx
import { registerComponentDecorator } from '@workflowbuilder/sdk';
...
[Previous Custom JsonForms control](https://www.workflowbuilder.io/docs/guides/custom-jsonforms-control/)
[Next Use Variable Picker](https://www.workflowbuilder.io/docs/guides/use-variable-picker/)
