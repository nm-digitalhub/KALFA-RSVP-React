...
`schema.ts` is a [JSON Schema](https://json-schema.org/) describing the shape of a node’s `properties` — the data half. For the visual half (how those properties render in the property panel), see [Form overview](https://www.workflowbuilder.io/docs/node-schemas/form-overview/).

The SDK’s [`NodeSchema`](https://www.workflowbuilder.io/docs/api/types/nodeschema/) type narrows the standard JSON Schema vocabulary to the subset the editor understands, plus a handful of SDK-specific additions (notably `options` for select-style fields).
...
1. **Rendering** — JsonForms uses the schema (combined with the matching [`UISchema`](https://www.workflowbuilder.io/docs/node-schemas/form-overview/)) to know what each control should accept.

1. **Type inference** — `NodeDataProperties<typeof schema>` extracts a precise TypeScript type for the node’s `properties`, so consumers of `NodeData` get autocomplete on their custom fields.
...
See the [`Option`](https://www.workflowbuilder.io/docs/api/types/option/) API reference for the exact shape.
...
For conditional shape changes (e.g. *if `method === 'POST'`, then `body` is required*), use the standard JSON Schema `if` / `then` / `else` keywords inside `allOf` — see [`NodeSchema`](https://www.workflowbuilder.io/docs/api/types/nodeschema/) for the supported subset.
...
```
Build the scope string from a typed dot-path with [`getScope`](https://www.workflowbuilder.io/docs/api/forms/getscope/) to get autocomplete and rename safety.
...
[`sharedProperties`](https://www.workflowbuilder.io/docs/api/utilities/sharedproperties/) — the canonical `label` + `description` pair. Spread it into every `NodeSchema['properties']` so your nodes pick up display title and subtitle without redeclaring them.
```ts
import { type NodeSchema, sharedProperties } from '@workflowbuilder/sdk';
...
```
[`statusOptions`](https://www.workflowbuilder.io/docs/api/utilities/statusoptions/) — the three canonical statuses (`active` / `draft` / `disabled`) with matching status icons. Use it as the `options` array on a `status: { type: 'string' }` field.
```ts
...
[`generalInformation`](https://www.workflowbuilder.io/docs/api/utilities/generalinformation/) — pre-built `Accordion` rendering Title / Status / Description for the three fields above. Drop it as a top-level element inside your node’s `uischema.elements`.

[`globalControls`](https://www.workflowbuilder.io/docs/api/utilities/globalcontrols/) — UISchema fragments rendered on every node regardless of type (today: the missing-previous-variable error slot used by the validation plugin). Spread the array into your node’s top-level `elements` so the diagnostic surface stays consistent across node types.
```ts
...
- [Add a custom node](https://www.workflowbuilder.io/docs/guides/add-a-custom-node/) — full walk-through using a `Webhook` example

- [Form overview](https://www.workflowbuilder.io/docs/node-schemas/form-overview/) — overview of the visual half

- [Form controls](https://www.workflowbuilder.io/docs/node-schemas/form-controls/) — every control type and its props

- [`NodeSchema`](https://www.workflowbuilder.io/docs/api/types/nodeschema/), [`NodeDataProperties`](https://www.workflowbuilder.io/docs/api/types/nodedataproperties/), [`Option`](https://www.workflowbuilder.io/docs/api/types/option/) — auto-generated type reference

- [JSON Schema specification](https://json-schema.org/) — the underlying standard

[Previous Node schemas](https://www.workflowbuilder.io/docs/node-schemas/)
[Next Form overview](https://www.workflowbuilder.io/docs/node-schemas/form-overview/)
