...
## 5c. Environment-parameter provisioning (owner directive 2026-08-23 — the wizard
DOES handle env vars; only their VALUES stay out of its state/logs)

Correction of the earlier "secrets are manual" stance: like any real installer, the
wizard collects configuration through a form — the security rule is narrower: secret
values travel browser→server over TLS and land in `.env.local` (0600) directly; they
never enter the wizard's state file, logs, terminal output, or prompts.

Two provisioning paths, chosen automatically:

1. **Fresh server (no panel yet) — WordPress-style, on the REAL domain** (owner
   refinement 2026-08-23): by the time params are needed, install order has already
   delivered DNS → TLS cert → vhost, and the vhost proxies the target domain to
   127.0.0.1:3002 — a port nothing occupies yet (the app is not built). The wizard's
   temporary setup form binds EXACTLY there, so the operator simply opens
   `https://<target>/setup?token=<one-time>` in a normal browser — TLS'd, no SSH
   tunnel. Form renders the missing keys (names from `.env.example`, grouped, RTL
   Hebrew), POSTs once; the wizard writes `.env.local` 0600, shuts the form down,
   builds, and starts pm2 on the same port — the real app replaces the form.
   Security (commercial-installer standard): single-use token in the URL printed only
   to the terminal, constant-time comparison, one submission, auto-shutdown — no
   permanent setup surface; the window exists only during install minutes.
2. **Running app (relocation / repair)** — `/admin/relocation/setup`, behind
   `requirePlatformOwner`: shows ONLY missing keys (never existing values), writes to
   `.env.local`; overwriting an existing key requires an explicit per-key confirm.

Closing the loop: step I4 enters `waiting-external` kind `env-provisioning` (in the
contract + labels) and polls `missingEnvKeys()`; the moment the last key lands, the
wizard resumes on its own — no return to the terminal needed. /admin/relocation shows
the waiting state with a link to the form. Implementation belongs to the mutation
phase (the temporary form server + the admin setup page are both writers).

**Parameter VALIDATION (owner requirement 2026-08-23; every probe live-doc-verified
that day):** the form does not accept a value on format alone — each credential gets a
real, read-only "test connection" probe run SERVER-side (values never echo back, never
logged; 5–8s timeout), with per-key spinner → ✓/✗ and a Hebrew reason that
distinguishes the failure classes: «המפתח נדחה על-ידי השירות» (auth) vs «אין תקשורת
לשירות — ייתכן שזו בעיית רשת, לא המפתח» (network). Probe catalog:

| Keys | Probe | Verified source |
|---|---|---|
| Supabase URL + anon/publishable | `GET {url}/auth/v1/settings` w/ apikey → 200; 401=bad key | supabase repo auth-keys test + kong ACL |
| Supabase service-role | `GET {url}/auth/v1/admin/users?per_page=1` → 200; 401/403=not admin | kong ACL (admin consumer) |
| Supabase DB (pooler) | pg connect + `SELECT 1`; 28P01=bad auth vs ECONN=network | repo worker pooler |
| MS Graph (tenant/client/cert) | local cert parse → client-credentials token POST; AADSTS codes=bad cred | learn.microsoft.com client-creds flow |
| Graph mailbox | `GET /users/{mailbox}/mailFolders?$top=1`; 403=permission, 404=mailbox | same + repo graph-client |
| Resend | `GET /domains` → 200; 401=bad key | resend docs |
| ElevenLabs | `GET /v1/user/subscription` (xi-api-key) | elevenlabs-js reference |
| Meta app+tokens | `debug_token` (is_valid) + `GET /{ig-id}?fields=id` | developers.facebook.com (read-only edge) |
| Voximplant creds file | repo core.ts GetAccountInfo (read-only by design) | repo |
| GA4 SA + property | Admin API `properties.get` (readonly scope) | developers.google.com |
| VAPID pair | LOCAL: `webpush.setVapidDetails()` throws on bad pair — no network; pair is GENERATED by the wizard (`web-push generate-vapid-keys`), only SUBJECT is user-entered | installed web-push vapid-helper.js |
| SUMIT | **no documented read-only endpoint** (checked live; all repo calls are charge-family) → format-only + explicit «דלג — אאמת ידנית» recorded in openItems; manual check via existing /admin/sumit-test | live check 2026-08-23 |
| App-internal secrets (webhook/encryption/console tokens) | format-only (length/encoding per consumer) | repo consumers |
| Origins/enums/ids/flags | zod format rules | — |

Form rules: "בדוק הכול" runs probes in parallel; submit disabled until every required
key passes format+probe or carries an explicit per-key skip (skips land in the run's
openItems so /admin shows what was accepted unverified). ExtrA SMS + WhatsApp Cloud
keys are NOT env keys — they live in `app_settings` (DB), out of this form's scope —
the form links to /admin/settings instead (step I12 polls their presence).
Prerequisite closed 2026-08-23: `.env.example` regenerated to cover all key names
(it is the form's key catalog).

Field-rendering rules by catalog kind (env-validation.ts):
- `generated` (VAPID pair): NEVER rendered as inputs — the wizard generates the pair
  itself (before the build; the public key is build-inlined) and the form shows a
  read-only «נוצר אוטומטית ✓» row; only VAPID_SUBJECT is a real field.
- `optional` (SUPABASE_ACCESS_TOKEN): rendered ONLY when the resolver finds no value in
  env NOR in `~/.supabase/access-token` (the linked CLI's credential — discovered
  2026-08-23); when shown, labeled «נדרש לאשף בלבד, לא לריצת האתר».
- `skippable` (SUMIT): normal field + explicit «דלג — אאמת ידנית» control.
- `db-settings`: never in this form — admin screens only.
