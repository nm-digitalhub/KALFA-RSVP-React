...
### 1. ~~פריסה~~ ✅ נפרס ונדחף 13.9 13:11

כל הקומיטים עד `100bb42` חיים (`.deploy-id` = `mtznjkne`, build 13:11:29, `kalfa-beta` restart #92). ה-worker **לא** הופעל מחדש — `dist/worker.cjs` ללא שינוי, נכון, כי שום שינוי מהאצווה הזו אינו נוגע בחבילת ה-worker. `seo:audit` 12/12 בציון A. יומן השגיאות של `kalfa-beta` — הרשומה החדשה ביותר היא מ-11.9, כלומר אפס שגיאות חדשות מהפריסה.

**אומת חי אחרי הפריסה:** חמשת המסלולים עונים נכון — `/admin/integrations`, `/admin/integrations/meta-whatsapp` ו-`/admin/templates` מחזירים 307 להתחברות (כלומר גרף המודולים **נטען**, מה ששולל את מחלקת הכשל של `server-only` שנכנס לרכיב לקוח — היא הייתה 500 בטעינת מודול), ו-`/admin/channels` + `/admin/alerts` מפנים ליעדים החדשים. הסמלים החדשים נמצאים בארטיפקט שנפרס (`SendPolicyForm` ב-`meta-whatsapp/page.js`, `observed_category` ב-`templates/page.js`, שני ה-actions ב-`server-reference-manifest.json`).

**הערך החי של `whatsapp_send_policy` נבדק** — הוא **זהה** ל-`DEFAULT_SEND_POLICY` ועובר את `parseSendPolicy`, כך שהכרטיס יציג "מדיניות שמורה" (ירוק) עם שלוש שורות מועדפות (7/3/1) ועוד שורה ריקה. לא יופיע מצב `invalid`.
...
### 3. מעבר בדפדפן — **מה שנשאר לא מאומת, אחרי הפריסה**

התשובה ב-307 מוכיחה שהמודולים נטענים; היא **אינה** מוכיחה רינדור. מה שדורש מושב מחובר:

- **`/admin/integrations/meta-whatsapp`** — `SendPolicyForm` (רכיב `'use client'` חדש): 6 שורות ימים ×2 שדות שעה, שורת שבת הנעולה, והשמירה עצמה. גם `MetaStatusCard` מעליו.
- **`/admin/integrations`** — `ChannelCatalogEditor`, שנחת עם Task 0.6 Step 4b ומעולם לא רונדר.
- **`/admin/templates`** — **הפריט החשוב ביותר ברשימה.** כפתור אישור הקטגוריה הוא action **שני** שנשלח מתוך טופס שכבר יש לו אחד, דרך `formAction`. הבדיקות ממקקות את `useActionState`, כך שלאינטראקציה הזו אין שום כיסוי ריצה. שלוש התבניות עם ה-drift (`gift`, `thankyou`, `sales_signup_link`) הן מקרה המבחן.
- **RTL** בכל השלושה — רשתות השעות והשורות החדשות.

**תג ה-Debug:** הקומיט `6cde32f` הוסיף 12 תורי cron לניטור הישנוּת. כל השתים-עשרה נמדדו ב-13.9 ואף אחד אינו מאחר, כך שהתג לא אמור להשתנות בפריסה. אם הוא יאדים בהמשך — זה הניטור עובד, לא הפריסה נשברת.
...
### שתי שאלות ששוחררו יחד עם הטופס, ונבדקו מול המתזמן

`parseSendPolicy` מאמתת את החלון ואת הפריסה **בנפרד** — שום כלל לא דוחה `spreadSpanMs` רחב מהחלון שהוא אמור להיכנס אליו. הטופס החדש הופך את זה לנגיש בשתי לחיצות (שישי 09:00–10:00 + פריסה של 90 דק׳), כך שהשאלה מי מטפל בזה הפכה רלוונטית.

**נמדד בקוד, לא הוסק:** `spreadWithin` (`send-window.ts:116-126`) כותבת `room = Math.max(0, Math.min(spanMs, windowEndMs - startMs))` — הפריסה **נחתכת** לאורך החלון, ובקריאה עצמה `usableEnd` הוא כבר המוקדם מבין סגירת החלון, כניסת החסימה הבאה והתפוגה. פריסה רחבה מהחלון היא לכן no-op, לא חריגה. **אין צורך בכלל נוסף ב-`parseSendPolicy`**, וטוב שכך: כלל ברמת הטופס היה הופך לשכבה רביעית שממילא לא מכסה את מסלול ה-SQL שהיה הכותב היחיד עד היום.

אותה שאלה ל-`preferredTimeByDaysBefore`: שעה מועדפת 03:00 עוברת את בדיקת הפורמט. **נמדד:** לולאת `resolveSendSlot` (`send-window.ts:281-284`) מתחילה ב-`max(planned, now)` ומקדמת `if (t < windowStartMs) t = windowStartMs` — שעה לפני החלון נדחפת לתחילתו. גם כאן אין ליקוי.

### כל 12 התורים שנוספו לניטור — נבדקו, כולם בריאים

הוספה למפה הופכת תור מ"לעולם לא מסומן" ל"מסומן כשהוא מאחר", ולכן פריסה יכולה להפוך תג ירוק לאדום. נמדד מול `pgboss.schedule` + `pgboss.job_common` ב-13.9 09:24 UTC — **אף אחד מהשתים-עשרה אינו חורג מהמרווח שקיבל**:

| תור | גיל בדקות | מרווח |
|---|---|---|
| `graph-intake-subscription-renew` | 0 | 1,080 |
| `inquiry-followup-sweep` · `fleet-request-expire-sweep` · `console-agent-calendar-presence-sync` | 3 | 15 / 30 / 30 |
| `sumit-hold-reconcile` | 23 | 90 |
| `signup-reminder-sweep` | 123 | 4,320 |
| `unconfirmed-cleanup-sweep` · `archive-maintenance-sweep` · `agreement-archive-sweep` · `whatsapp-template-health-sync` | 453–528 | 4,320 / 14,400 |
| `instagram-token-refresh` | 7,686 | 14,400 |
| `archive-backup-sweep` | 9,956 | 57,600 |

כלומר התג לא ישתנה בפריסה. אם הוא **כן** יאדים בהמשך — זה הניטור עובד, לא הפריסה נשברת.
