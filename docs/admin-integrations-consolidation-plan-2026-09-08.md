...
## ⚑ מה מחכה לבעלים (מעודכן 2026-09-13, אחה"צ)

הרשימה המרוכזת של כל מה שאי אפשר להתקדם בלעדיו. כל השאר בתוכנית ממשיך בלי הבעלים.

### 1. פריסה — תשעה קומיטים על הענף, אף אחד לא חי

הענף `feat/admin-integrations-consolidation` מכיל תשעה קומיטים שלא נפרסו: `04e7e47`, `5e87032`, `d02141a`, `42fc265`, `87e4686`, `0f432fb`, `5e0ec41`, `6cde32f`, `96b3a79`. השרת החי מריץ עדיין את בניית 11:26 של 13.9.

### 2. שיוך `whatsapp_import_sender` — חוסם את כל האימות של Phase 1.5

`/admin/integrations/numbers` → סנכרון → לשייך את התפקיד ל-**+972 3-330-1505**. עד שזה נעשה, ניתוב הייבוא לפי מספר מקבל **לא פעיל בפועל** (המסלול נופל חזרה להתנהגות הישנה, בכוונה), וארבע הבדיקות החיות של §1.5.4 אינן יכולות לרוץ.

### 3. מעבר בדפדפן אחרי הפריסה — **דבר שאי אפשר לאמת מכאן**

`/admin/integrations` ו-`/admin/integrations/meta-whatsapp` **מעולם לא רונדרו בצורתם הנוכחית**. שניהם מרכיבים עכשיו רכיבי `'use client'` חדשים עם `useActionState` (`ChannelCatalogEditor`, `SendPolicyForm`), והשרת החי נבנה לפני ששניהם נחתו. `build` עובר ובדיקות עוברות — ואף אחד מהם אינו אותה בדיקה: הזיכרון `verification-gate-runtime` קיים בדיוק כי שערים סטטיים כאן החמיצו כבר חריגות של גבול שרת/לקוח. גם `/admin/templates` — כפתור אישור הקטגוריה החדש.

### 4. הכרעות שדורשות אישור לפני שנכתבת שורה

| מה | למה חוסם |
|---|---|
| **Phase 3** (Voximplant) | Task 3.1 מודדת את תפקיד ה-service account ב**קריאות חיות** עם אישורים; Task 3.3 היא רכישת DID ש**מוציאה כסף אמיתי** |
| **Phase 4** (caller id לכל פרסונה) | דורשת עמודות חדשות ב-`app_settings` → מיגרציה |
| **Task 5.1** (בריאות וריאנטים) | דורשת את טבלת §4.3 → מיגרציה |
| **Task 5.4** (כיסוי webhooks) | קריאת Graph חיה; ה-`DELETE /{app-id}/subscriptions` עדיין **UNVERIFIED** (§8.6) |
| **Phase 6** | החלטה D9, ואישור israeli-compliance-advisor על יצירת בקשת חזרה משיחה נכנסת |

### 5. פעולה ידנית ב-WhatsApp Manager

למחוק את `+3333756982370` — מספר בדיקה שנשאר תפוס בחשבון.
...
## 0.11 סטטוס 2026-09-13 (אחה"צ) — Phase 5 כמעט סגורה: 5.2, 5.3, 5.5 בוצעו; 5.6 כבר הייתה בנויה

ארבע משימות מ-Phase 5 נסגרו בשלושה קומיטים, בלי שום דבר שדורש את הבעלים — לא מיגרציה, לא קריאה חיה, לא כסף. מה שנשאר ב-Phase 5 הוא **5.1** (בריאות וריאנטים — דורשת את מיגרציית §4.3) ו-**5.4** (כיסוי webhooks — קריאת Graph חיה, וה-DELETE עדיין UNVERIFIED לפי §8.6).

### Task 5.6 לא נבנתה כי היא כבר קיימת

לפני שנכתבה שורה: `src/lib/sms/extra-client.ts` (עם `getAuthKey`), `src/lib/sms/run-key-check.ts`, `QUEUES.extraKeyCheck` מתוזמן ב-`worker/main.ts` ל-`20 4 * * *`, ושורת ExtrA ב-`ops/integrations.ts` כבר מצביעה עליו. `docs/extra/openapi-extra-v1.json` + `README.md` קובעים את המפרט. **המשימה סגורה; מה שנשאר ממנה זה כרטיס העמוד עצמו (Step 2 של Task 1.2), לא הניטור.**

### Task 5.3 — `whatsapp_send_policy` יצא מה-SQL

DAL `getSendPolicyForAdmin/updateSendPolicy` (`manage_settings`, cookie client) + `sendPolicyFromFormData` (טהור) + `updateSendPolicyAction` + טופס בעמוד Meta. סוגר את **G9** ואת פסיקת הבעלים "מתג במסד בלי שליטה בפאנל אינו גמור".

**התקרות לא הוגדרו מחדש.** `parseSendPolicy` מחזיקה אותן ונקראת בשלוש שכבות — הפרסר הטהור, ה-action, וה-DAL. ה-DAL מאמת **שוב** כי action אינו הקורא היחיד שמודול נתונים יכול לקבל אי פעם. אפשר רק **לצמצם**: רצפה 09:00, תקרה 20:30 (שישי 12:00), hardCap 21:00, motzash ≥60 דק׳, שבת = אין שליחה.

**שבת אינה שדה.** הטופס אינו מרנדר קלט לשבת, והפרסר מתעלם מכל ערך שמגיע תחת השמות האלה — POST מעובד ביד לא פותח שליחה בשבת. גם **לא** disabled: פקד מושבת אומר "אתה לא רשאי", ומזמין את השאלה "אז מי כן". אף אחד. זה שינוי קוד.

**שני מקומות שבהם הסקיצה בתוכנית הייתה מייצרת ליקוי:**

| מה התוכנית כתבה | מה נבנה, ולמה |
|---|---|
| שלוש שורות קבועות ל-`preferredTimeByDaysBefore` (7/3/1) | שלוש הן המפתחות שהערך החי **במקרה** נושא. רינדור של בדיוק שלוש מוחק בשקט מפתח רביעי בשמירה הבאה — ולוח הפניות הוא לכל אירוע, כך שנקודת מגע יכולה לשבת בכל `days_before`. השורות נגזרות ממה ששמור, ועוד שורה ריקה להוספה. מחיקת השעה מסירה שורה. |
| אין מצב ל"יש ערך שמור, והשולח דוחה אותו" | `getSendPolicy()` בולעת את הכשל ומתזמנת לפי ברירת המחדל — כלומר הפאנל היה מדפיס מדיניות שאיש אינו מציית לה. `getSendPolicyForAdmin` מפרידה `stored / default / invalid`, וה-invalid מנמקת ומציגה **את מה שהשולח באמת משתמש בו**. |

**מיקום:** עמוד Meta/WhatsApp — העמודה היא `whatsapp_send_policy` ו-WhatsApp הוא הערוץ היחיד ששולח לפיה היום. הכותרת אומרת "שעות פנייה" ולא "וואטסאפ" כי ה-worker מתזמן **כל** שליחת קמפיין ממנה.

### Task 5.2 — הפאנל התעלם מ-12 מתוך 32 תורי ה-cron

התוכנית ביקשה **רשומה אחת** (`whatsapp-template-health-sync`). מדידה לפני הכתיבה: ה-worker מתזמן **32** תורים, ו-`QUEUE_EXPECTED_MAX_MINUTES` ידע על **20**.

זה לא קוסמטי: `isQueueStale` מחזירה **false** לכל תור שאינו במפה. כלומר כל אחד מהשנים-עשר יכול היה להפסיק לרוץ, והתג ב-Debug היה נשאר ירוק לנצח. ביניהם `whatsapp-template-health-sync` (ההשוואה הלילית מול Meta), `graph-intake-subscription-renew` (שתקיעה שלו מסיימת קליטת דואר נכנס), `inquiry-followup-sweep` ו-`archive-backup-sweep`.

כל השתים-עשרה נוספו, כל אחת ב-~3× הקצב שלה ומקובצת למשפחה. החודשי (`archive-backup-sweep`) קיבל דרגה משלו — 3× חודש הוא רבעון — **40 יום**: עובר את החודש הארוך ביותר, ומספיק צר שסנכרון חסר יתגלה תוך שישה שבועות.

**והתיאור הפסיק להיות תאריך.** ההערה במפה אמרה "verified 31.07" — זה מה שמראה מתוחזקת-ביד מתדרדרת אליו. `queue-schedule.test.ts` קורא עכשיו את `worker/main.ts` ואת `queues.ts` ונכשל **בשני הכיוונים**: תור מתוזמן בלי מרווח, ומרווח לתור שאיש אינו מתזמן (שינוי-שם שלא הושלם, שמנטר כלום). הוא גם מאמת שהחילוץ עצמו מצא ≥30 תורים — regex שמפסיק להתאים היה הופך כל שאר הטענות לריקות, כלומר השער היה עובר הכי חזק בדיוק כשנשבר.

**מה שלא שונה, ובמכוון:** `lastCheckedAt` של כרטיס WhatsApp. התוכנית ביקשה להפנות אותו לסנכרון התבניות היומי; הוא מצביע כיום ל-`whatsapp-health-check` **השעתי**, שמפעיל את הטוקן ואת צומת המספר. זו התשובה הטובה יותר ל"האם האינטגרציה חיה", והזזה לסנכרון יומי הייתה רגרסיה בלבוש של צעד בתוכנית.

### Task 5.5 — אפשר סוף-סוף לאשר קטגוריה ש-Meta לא תחזיר

שלוש תבניות (`gift`, `thankyou`, `sales_signup_link`) ביקשו UTILITY ומסווגות MARKETING מאז לפני שההתראה נולדה. Meta מסווגת לפי **גוף ההודעה**; שום דבר בפאנל אינו משנה את זה. מה שגם לא היה אפשרי זה לומר "אנחנו יודעים ומקבלים" — ולכן התג האדום הפך לרהיט, וכך בדיוק הרביעית הייתה עוברת בשקט.

`acknowledgeTemplateCategory(id, observed)` כותבת את הקטגוריה של Meta ל-`requested_category`: התג מתנקה, ההתראה הלילית נפסקת, ו**מעבר עתידי של Meta מתריע מחדש** כי הוא transition חדש. החיוב והמגבלות נשארים, והעתק של הכפתור אומר זאת — תג שמתנקה אסור שייקרא כבעיה שנפתרה.

**שני דברים שהשורה בתוכנית לא כיסתה:**

1. **הכתיבה נעוצה לקטגוריה שהייתה על המסך.** הסנכרון הלילי יכול לנחות בין טעינת העמוד ללחיצה. הערך שנצפה נשלח עם ה-id, נבדק מול הקריאה, **וגם** מצמצם את ה-UPDATE (`.eq('category', observed)`). הבדיקה קיימת פעמיים בכוונה: הקריאה מצמצמת את המרוץ, ה-UPDATE הנעוץ סוגר אותו.
2. **הכפתור אינו טופס מקונן.** הכרטיס כבר **הוא** טופס, וטופס בתוך טופס הוא HTML לא חוקי — ולכן `formAction` על כפתור submit, שגם מסביר למה ה-id שהוא צריך כבר שם.

**שערים (שלושת הקומיטים):** `lint` ✓ · `tsc` ✓ · `worker:deps` ✓ (331 מודולים) · `build` ✓ · **5,146 בדיקות עוברות, 23 מדולגות** (מ-5,075). שבע רגרסיות הוזרקו — קריאת שבת מהטופס, השמטת ההמרה דקות→מילישניות, מחיקת רשומה מהמפה, רשומה יתומה, regex שאינו מתאים לכלום, שינוי שם השדה הנסתר, והסרת הנעיצה מה-UPDATE — כל אחת הופלה.

**תיקון צדדי:** `data_access_expires_at` בשלוש חבילות בדיקה היה `1764633600` = **2025-12-02**, תאריך שכבר עבר, בעוד שהדדליין האמיתי של הטוקן החי הוא **2026-12-07** (§8). קורא עתידי היה יכול לקחת את הפיקסצ׳ר לתאריך האמיתי.

---

## 0.10 סטטוס 2026-09-13 — Task 2.5 בוצעה: הפאנל שואל את Meta, לראשונה

`getMetaStatus()` + `MetaStatusCard` בעמוד Meta. סוגר את **G7** (אף קריאה ל-`debug_token` לא הייתה קיימת בקוד) ואת החצי הקריא של **G8**.

**למה זה לא "מוגדר" עוד פעם.** `configured` נגזר מהעמודות שלנו — phone-number-id קיים, טוקן קיים — כלומר "מישהו הקליד ערך", והוא נשאר true לנצח. טוקן System-User שחלון הגישה שלו פקע **שומר על הצורה שלו**, ולכן הפאנל היה ממשיך לדווח ערוץ תקין בזמן שכל שליחה נכשלת. שתי שאלות שונות, ומוצגות זו לצד זו.

### שלושה מצבים, ובכוונה לא שניים

| מצב | מה מוצג |
|---|---|
| Meta ענתה | תקף / לא תקף + **הנימוק של Meta עצמה**, שתי תפוגות, ואילו הרשאות חסרות בשמן |
| לא הצלחנו לשאול | **"לא נבדק"** + הסיבה (אין app secret / אין app id / Meta לא זמינה) |
| לא מוגדר | "לא מוגדר" — לא תקלה ולא אזהרה |

מיזוג השניים האחרונים הוא בדיוק הליקוי שהצורה הזו קיימת כדי למנוע: "לא הצלחנו לשאול" שמוצג כ"הטוקן פסול" שולח אדמין להחליף טוקן תקין. שתי בדיקות מצמידות את ההפרדה, ב-DAL וברכיב.

### אומת מול התיעוד החי לפני שנכתבה שורה

`GET /{version}/debug_token` (reference רשמי, נקרא 13.9): GET; `input_token` = הטוקן הנבדק; `access_token` חייב להיות **טוקן אפליקציה** (`{app-id}|{app-secret}`) או טוקן של מפתח באפליקציה — ולכן צריך את מזהה האפליקציה ולא רק את הטוקן. השדות `is_valid`, `expires_at`, `data_access_expires_at`, `scopes` קיימים כולם במפרט.

**מה שהתיעוד לא אומר, ונרשם כדי שלא יומצא כעובדה:** אין ב-reference שום משמעות ל-`expires_at = 0`. המוסכמה היא "ללא תפוגה", וכל טוקן System-User ארוך-טווח מחזיר אותה, אבל זו **INFERRED**. הכרטיס כותב "ללא תפוגה (Meta מחזירה 0)" — מתאר את הערך, לא מכריז על כלל שמטא לא פרסמה. הדדליין האמיתי הוא `data_access_expires_at`, והוא מוצג בשורה נפרדת.

### המיגרציה של §4.4 לא נדרשה — וסדר העדיפות נכתב כבר עכשיו

התוכנית קבעה ש-`debugToken` דורש עמודה חדשה `app_settings.whatsapp_app_id` (§4.4, Phase 5). **מדידה: `META_APP_ID_WA` כבר קיים ב-env**, ומזהה אפליקציה אינו סוד (הוא גלוי בכל כתובת OAuth). לכן אין כאן מיגרציה. `resolveAppId()` קורא **קודם** מ-`app_settings` ורק אחר כך מה-env, למרות שהעמודה לא קיימת — `select('*')` פשוט משמיט עמודה שאינה שם, אותה תבנית קדימה-תואמת ש-`getWhatsAppConfig` מתעדת. כך שהוספת העמודה בעתיד היא מיגרציה ותו לא, בלי נגיעה בקוד. בדיקה מצמידה את הסדר.

**שומר שנוסף מהמדידה:** כשל בקריאת `app_settings` **נופל חזרה ל-env** במקום לדווח "אין app id" — תקלת מסד לא נראית כמו תצורה חסרה.

### מה שלא נבנה, ובכוונה

התוכנית ביקשה גם השוואה מול `META_LATEST_GRAPH_VERSION`. **לא נבנתה:** שום דבר כאן אינו יכול לקרוא את הגרסה העדכנית של Meta חי, וקבוע קשיח הופך לשקר בחודש שבו Meta משחררת גרסה (כלל הבית: אין placeholder שמזדקן). הכרטיס מציג את הגרסה הנעוצה כעובדה פשוטה עם קישור ל-changelog. בדיקה מצמידה שהכרטיס **אינו** טוען "עדכנית".

### סודות

`debug-token.ts` מקבל את שלושת הערכים כארגומנטים, לא כמצב-מודול, ואף אחד מהם אינו מופיע בשום הודעת שגיאה — כולל המחרוזת `{app-id}|`. `getMetaStatus` מחזיר בוליאנים, חותמות זמן ושמות הרשאות בלבד. שתי בדיקות סורקות את כל הענפים ומאמתות שאף סוד אינו דולף.

**שערים:** `lint` ✓ · `tsc` ✓ · `build` ✓ · **5,075 בדיקות עוברות, 23 מדולגות**. שש רגרסיות הוזרקו על פני שלושה קבצים — כל אחת הופלה. שער הכיסוי (`admin-data-layer-coverage`) הפיל את המודול החדש בהרצה הראשונה עד שסווג, כמתוכנן.

**נשאר ב-Phase 2:** רק האימות החי, שחסום על קו שמקבל SMS ואין עליו חשבון WhatsApp. `debug_token` על הטוקן החי הוא בדיקה בטוחה שאפשר להריץ מיד — היא קריאת GET בלבד ואינה נוגעת בתקציב 10/72h.
