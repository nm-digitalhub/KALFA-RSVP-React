...
**מגבלה מוצהרת:** היעד יושב באותו פרויקט Supabase כמו אחד המקורות. זה מגן מפני מחיקה או שחיתה של הספרייה ב-SharePoint ומפני מחיקה של אובייקט ב-`id-documents`, אבל **לא** מפני אובדן הפרויקט כולו.

**עותק שלישי מחוץ ל-Supabase — נדחה (הכרעת בעלים 6.9.2026).** הוצע דלי אצל ספק שלישי עם נעילה נגד מחיקה והצפנה מקומית; הבעלים הכריע שאין צורך. אין לחזור ולהציע. אם המצב ישתנה (למשל אירוע אובדן נתונים, או דרישה של יועץ משפטי), זו נקודת הפתיחה.

**ממצא נלווה, לא קשור לגיבוי:** פרויקט ה-Supabase רץ ב-`ap-south-1` (מומבאי). חוות הדעת ב-§5 קבעה שתקנות העברת מידע לחו"ל אינן חלות, בהסתמך על כך שנתוני Microsoft 365 נשמרים בישראל — וזה נכון ל-SharePoint. אבל מסד הנתונים הראשי, ובתוכו כל פרטי האורחים והלקוחות, יושב מחוץ לישראל ממילא. **שאלה לעורך הדין**, עצמאית לחלוטין משאלת הגיבוי.
