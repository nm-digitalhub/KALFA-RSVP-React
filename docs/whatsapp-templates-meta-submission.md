...
**הופעל (2026-07-03, בהוראת המנהל):** 4 שורות ה־WhatsApp עודכנו ל־
`active=true`, ושורת `invite` הוחלפה ל־`kalfa_event_invite_v2` (UTILITY).
`call_1` נשארה כבויה (אין תשתית שיחות). ניהול המתג זמין ב־/admin/templates
(checkbox קיים). בעת ההפעלה אומת שאין נגיעות שמועדן הגיע (sink ריק; הקמפיין
הפעיל היחיד — חתונה 22.7) — חובה לפרוס את קשירת הפרמטרים לפני המועד הראשון,
אחרת שליחה תצא חשופה ותידחה על אי־התאמת פרמטרים.
