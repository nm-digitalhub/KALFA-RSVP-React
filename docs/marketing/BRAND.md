...
**משפט מיצוב (✔ אושר ע"י הבעלים 24.8.2026):** "כל האורחים מאשרים הגעה — בלי שתשלחו 200 הודעות ידניות."
...
- מודל חיוב (עדכני 24.8.2026, מקור: `src/lib/fleet/business-facts.ts`): **בסיס +
  חריגה (base+overage)** — דמי הפעלה חד-פעמיים לאירוע הכוללים מכסת אנשי קשר
  שנענו, ותוספת לכל נענה מעל המכסה. **⚑ מחיר מדויק** = מ-admin, לא מקובע בטקסט.
  - **כלל תאימות (סקירת 26.7, פריט עו"ד 18):** דמי ההפעלה נגבים ללא תלות בתוצאה
    — אסור להציגם כמותנים. הטענה "משלמים רק על מי שנענה" נכונה **רק** לגבי
    התוספת שמעל המכסה, לעולם לא לגבי הבסיס.
  - **יחידת החיוב:** אנשי קשר **שנענו בפועל** (השיבו בוואטסאפ / השלימו שיחת AI)
    — לא אורחים ולא הודעות. אין לגזור מחיר ממספר אורחים; איש קשר אחד עשוי
    לייצג כמה אורחים באותו בית.
  - כשמנגנון הבסיס כבוי (gate), המודל בפועל הוא per-reached טהור — לוודא מול
    admin לפני כל פרסום.
