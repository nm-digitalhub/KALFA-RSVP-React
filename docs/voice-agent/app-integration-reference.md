...
### 6.8 `POST /api/agents/telemetry` — אבחון חי של המכשיר ✅ כבוי כברירת מחדל

ערוץ **אבחון**, לא משטח מוצר. האפליקציה שולחת את שלבי מסלול השיחה שלה, והשרת
מוסיף שורה אחת לכל אירוע לקובץ שאפשר לעשות עליו `tail -f` דרך SSH בזמן שהטלפון
מצלצל. הוא קיים מסיבה אחת: אין שום דרך אחרת לראות את המכשיר — בלי ADB, בלי USB,
בלי Wi-Fi, סלולר בלבד — ולכן כל אבחון של "השיחה נותבה והטלפון לא צלצל" נטען עד
היום מתוך קריאת קוד ולא מתוך ראיה.

```jsonc
// בקשה — עד 50 אירועים, גוף עד 64KB
{ "events": [
  { "at": "2026-08-15T04:12:33.412Z",   // שעון המכשיר. נרשם, לא נסמכים עליו לסדר
    "sid": "c7f3a91b",                  // p… = תהליך, c… = ניסיון שיחה אחד
    "seq": 42,                          // מונוטוני לכל תהליך; חור = שורות אבדו
    "name": "fcm.message_received",
    "fields": { "vox": "true", "keys": "3" } }
] }
// תשובה
{ "ok": true, "written": 1 }   // 202
```

- **הרשאה:** `requireConsoleAgent` (Bearer + `is_console_agent`), כמו
  `/api/agents/status`. נבדקת **לפני** ה-flag, כדי שאנונימי לא יוכל לבדוק אם
  הערוץ פתוח בשרת הזה.
- **flag:** `503` עד ש-`DEVICE_TELEMETRY_ENABLED=1` מוגדר בסביבת השרת. האפליקציה
  קוראת `503` כ"תפסיק לנסות לזמן מה", לא כשגיאה שכדאי לחזור עליה.
- **קצב:** 40 בקשות לדקה לכל נציג (`rateLimit` לפי `userId`, לא לפי IP — המכשיר
  מאומת ומחליף IP בסלולר).
- **בלי PII, לעולם.** הבדיקה רצה פעמיים ובאופן בלתי תלוי: האפליקציה מנקה לפני
  השליחה, ו-`looksLikePersonalData` **דוחה** במקום לסמוך עליה. ערך שהאפליקציה
  הייתה מצנזרת הוא ערך שהנתיב מחזיר עליו `400`, בלי להחזיר את הערך עצמו.
- **הקובץ:** `.telemetry/device-telemetry.log` תחת `cwd` (או
  `DEVICE_TELEMETRY_LOG_PATH`), כתיבה אחת לכל אצווה — תחת pm2 כמה worker-ים
  כותבים לאותו קובץ, ו-`O_APPEND` יחיד בטוח מפני שזירה בעוד לולאה לכל שורה אינה.
  מתגלגל ב-32MB ושומר דור אחד.
