...
| A-100 | **שלוש אינווריאנטות עברו מ-TypeScript למסד** (`20260914233659`): (1) אינדקס ייחודי חלקי ל-`voice_purpose_attempts`; (2) `call_attempt_id` מותר **רק** כש-`attempt_table` הוא `call_attempts`; (3) `linked_at` צמוד לזוג. ⚠️ **הסמנטיקה אומתה מול תיעוד PostgreSQL הרשמי, לא מקומי:** *"a check constraint is satisfied if the check expression evaluates to true **or the null value**"* — ולכן `attempt_table = 'call_attempts'` **לא מספיק** (עם NULL הוא מחזיר NULL ו**עובר**). `IS NOT DISTINCT FROM` *"never returns null"* והוא הכלי הנכון. אומת בסימולציה: 0 הפרות אחרי. | PostgreSQL docs + ספירה חיה |
