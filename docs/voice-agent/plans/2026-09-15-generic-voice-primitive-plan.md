...
#### 🚫 האילוץ שמעצב את הטופס — ולא ניתן לוותר עליו

לפי **A-13**, שדה שדגל הדריסה שלו **כבוי על הסוכן מפיל את השיחה בשגיאה**.
לכן בורר הסוכן ורשימת שדות הדריסה **כרוכים זה בזה**:

> בחירת סוכן → הטופס נשאל אותו אילו דריסות מותרות
> (`platform_settings.overrides.conversation_config_override`) →
> **מציג רק אותן.**

זה הופך את A-13 מבאג-בהמתנה לבלתי-אפשרי: אי אפשר להגדיר שדה שישבור את
השיחה, כי הוא לא יוצג. ⚠️ **ובלי זה, הצומת הופך כל שדה לרגל-פח.**
