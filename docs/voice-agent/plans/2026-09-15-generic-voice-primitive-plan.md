...
| A-97 | **Backfill מוכן ומאומת בספירה, לא הוחל.** `20260914232506_backfill_orphan_call_analysis_links.sql`: 10 שורות ל-`callback_request_attempts`, 4 ל-`sales_call_attempts`, 0 לשתי האחרות — **14 משוקמות, 8 נשארות יתומות** (אין להן ניסיון בשום טבלה: 4 RSVP מ-21.07, 2 Sales-Close, 2 שירות-לקוחות). אומת שאין `el_conversation_id` כפול באף טבלה, כך שה-UPDATE דטרמיניסטי. `call_attempt_id` לא נגוע — הוא מפתח זר ל-`call_attempts`. **ההחלה שלך.** | ספירת יבש על ה-DB |
