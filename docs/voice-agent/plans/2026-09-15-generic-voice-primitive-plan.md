...
| A-103 | ⚠️ **מלכודת תפעולית שנצפתה בפריסה הזו:** `.deploy-id` נכתב ב-02:40:49 אך `kalfa-worker` עלה ב-02:37:15 — **לפני** שהחבילה שלו נבנתה. Node טוען את `worker.cjs` פעם אחת לזיכרון בעלייה, כך שחבילה חדשה אינה מספיקה: **התהליך חייב לעלות מחדש אחריה**. הוורקר הוא שמעבד את `webhook_inbox`, ולכן עד שהוא עלה, כל התיקון לא רץ למרות ש"הפריסה הסתיימה". להשוות תמיד את זמן העלייה של התהליך מול mtime של החבילה. | נצפה בפועל |
