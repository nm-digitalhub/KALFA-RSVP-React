...
#### ✅ A-13 נאכף בשתי שכבות — ואחת מהן יכולת SDK שלא נוצלה

⚠️ **תיקון כפול לניסוחים קודמים שלי.** קודם כתבתי "הטופס יציג רק את
הדריסות המותרות"; אחר כך כתבתי שה-SDK אינו תומך בכך כלל. **שניהם לא
מדויקים.**

**שכבה 1 — `allOf` עם `if/then`, בזמן עריכה.** `NodeSchema` חושף
`allOf?: IfThenElseSchema[]`, ו-**הריפו אינו משתמש בו בשום מקום**:

```
SchemaCondition   = { properties: { <field>: { const: value } } }
ConditionalSchema = { properties: { <field>: FieldValidationSchema } }
StringFieldValidationSchema = { type:'string', minLength?, maxLength?, pattern?, format? }
```

כלומר ניתן לבטא: **אם `agentId` הוא סוכן שדגל ה-`prompt` שלו כבוי → הגבל
את `prompt` ל-`maxLength: 0`**. `buildPaletteItems` מייצר את מערך ה-`allOf`
**מדגלי הסוכנים החיים** — ענף אחד לכל סוכן. **כלום לא hardcoded.**

ⓘ זו וולידציה, לא הסתרה: השדה מוצג אך מסרב לערך. פחות אלגנטי מהסתרה, אבל
ילידי ל-SDK ונוצר מנתונים חיים.

**שכבה 2 — `arm-check.ts`, בזמן חימוש.** לא כפילות: היא תופסת מה שהראשונה
**לא יכולה** — צומת שהוגדר כשהדגל היה דלוק, ושהדגל כובה אחר כך. הסכמה
נצרבה בזמן בניית הפלטה; הדגל בשרת חי. **רק בדיקת החימוש קוראת את שניהם
באותו רגע**, והיא גם זו שאי אפשר לעקוף.
