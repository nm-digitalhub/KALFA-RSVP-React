...
| A-77 | ⚠️ **שני מזהי-משיבון על אותה שיחה.** `voicemail_detection` דלוק ב-RSVP, Meeting-Confirm ו-Sales-Close — **ולתסריטים שלנו שער AMD משלהם ב-VoxEngine**, fail-open, שכבר נכשל בשיחה אמיתית (`qa/call-6904838502`). שתי מערכות שמחליטות "זה משיבון" בלי לדעת זו על זו. לייעוד גנרי צריך להכריע מי הבעלים — ולפי A-53, בקרת השיחה היא של VoxEngine. | הקונפיגים + דוח QA |
...
| A-104 | ⚠️⚠️ **ראיה שמחדדת את A-8/A-9 ומטילה ספק בהנחת היסוד.** `RSVPAgent` שולח `conversation_config_override: {asr: {keywords}}` ודגל `asr.keywords` שלו **דלוק**. ואומת שהמערך **לעולם אינו ריק** — `asrKeywordsFor` מצרפת 21 מילות בסיס קבועות אחרי השמות, כך שכל ctx מוצלח מחזיר ≥21. ובכל זאת, בשתי שיחות ייצור (`conv_8101m1ye…`, `conv_3001m1xx…`) הרשומה מראה `conversation_config_override: {agent: null, asr: null, conversation: null, tts: null, turn: null}` — **בעוד 29 משתנים דינמיים מאותו פריים כן נרשמו**. אותה קריאה, שדה אחד נרשם והשני לא. | נמדד על שיחות חיות |
| A-105 | **מה זה אומר, ומה לא.** ה-MCP של Voximplant קובע: *"the connector passes the parameters object through to ElevenLabs as-is"* — **אין סינון בצד Voximplant**. אז או שהדריסה לא הוחלה, או שהרשומה אינה מהדהדת דריסות. ⚠️ **איני יודע איזה מהם**, והדוגמה הרשמית ב-`post-call-webhooks` לא מכריעה: היא מראה מבנה אחר (שתי מקטעים מקוננים) מזה שאנחנו מקבלים (חמישה מפתחות null). ⚠️ ו-`asr.keywords` הוא **אחד משני השדות** עם `soft-override-disallowed` (A-69), כך שייתכן שהוא מקרה מיוחד ולא הכלל. | MCP + התיעוד |
