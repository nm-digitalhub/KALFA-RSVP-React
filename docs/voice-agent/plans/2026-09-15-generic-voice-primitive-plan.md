...
| A-112 | ⚠️ **לא מתועד: האם `{{משתנה}}` נפתר בתוך `first_message` שנשלח כדריסה.** התיעוד מונה *"**First messages** to personalize greetings"* — אבל עבור ההודעה **המוגדרת בסוכן**. על מחרוזת שמגיעה כדריסה הוא שותק. ✅ **הגבול אומת משני הכיוונים:** ה-MCP של Voximplant קובע *"the connector acts as a **transparent WebSocket bridge** — your payload object is forwarded untouched"* ו-*"any dynamic_variables substitution … is handled **entirely on the ElevenLabs side**"*. כלומר זו שאלה של ElevenLabs בלבד. | שני ה-MCP + התיעוד |
