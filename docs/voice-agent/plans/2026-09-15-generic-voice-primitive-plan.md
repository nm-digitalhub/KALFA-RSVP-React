...
| A-69 | ⚠️ **`x-convai-soft-override-disallowed: true` — קוטביות לא ודאית, אל תסתמכו עליה.** מופיע על `asr.keywords` ו-`tts.supported_voices`. השם קורא הכי טבעי כ"התנהגות ה-soft-override **אסורה** לשדה הזה" — כלומר דריסה אסורה עליו **נדחית** ולא נבלעת בשקט, **ההפך** מ-A-13. בניסוח ראשון כתבתי את הקוטביות ההפוכה; אין לי ראיה לאף אחד מהכיוונים. **לאמת בשיחה אמיתית לפני שבונים על זה.** | הסכמה + סימון כלא-ודאי |
