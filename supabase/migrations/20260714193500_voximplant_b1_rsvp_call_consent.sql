-- B1 — capture contacts.call_consent_at from the public RSVP flow (Voximplant
-- AI-call channel). Additive: adds a trailing `_call_consent boolean` param to
-- submit_rsvp and stamps contacts.call_consent_at (monotonic, per-event) for the
-- contact behind the RSVP guest, and exposes a read-only consent boolean from
-- get_rsvp_by_token so the form can pre-check the box on repeat visits.
--
-- PERMISSIONS (mirrors the 2026-07-13 SECDEF-grant hardening,
-- 20260713211844_harden_secdef_function_grants_anon.sql): both functions are
-- SECURITY DEFINER + `SET search_path` and are called ONLY via the service_role
-- admin client (rsvp.ts submitRsvp/getRsvpByToken use createAdminClient). A
-- DROP+CREATE grants EXECUTE to PUBLIC by default, which would REGRESS that
-- hardening and re-expose the RPC to anon via /rest/v1/rpc — so we explicitly
-- REVOKE from public/anon/authenticated and GRANT only to service_role, matching
-- the pre-migration ACL {postgres, service_role}.

-- 1) submit_rsvp — new signature (trailing _call_consent), DROP+CREATE required
--    because a new parameter changes the function identity.
drop function if exists public.submit_rsvp(
  text, text, integer, integer, text, text, jsonb, boolean
);
...
create function public.submit_rsvp(
  ...
  _call_consent boolean default false
) returns jsonb
  language plpgsql
  security definer
...
begin
  ...
  -- (e2) B1: record CALL consent for the contact behind this guest. Placed AFTER
  --      all validation (so a rejected submit records nothing) and BEFORE the
  --      idempotency short-circuit (so a consent-only toggle is never swallowed).
  --      coalesce(call_consent_at, now()) is MONOTONIC + idempotent: never
  --      re-stamps, never revokes on a later unchecked submit. Scoped by
  --      (id, event_id) — consent is per (event, contact), not portable. No-op
  --      when the guest has no linked contact (contact_id null; B1.3 defers lazy
  --      materialization). Does NOT touch removal_requested or the DNC list.
  if coalesce(_call_consent, false) and _g.contact_id is not null then
    update public.contacts
       set call_consent_at = coalesce(call_consent_at, now())
     where id = _g.contact_id and event_id = _g.event_id;
  end if;
  ...
  --     NOTE: call consent is written above (unconditionally on a valid submit),
...
-- Preserve the hardened grant posture (NO anon/public; service_role only).
revoke execute on function public.submit_rsvp(
  text, text, integer, integer, text, text, jsonb, boolean, boolean
) from public, anon, authenticated;
grant execute on function public.submit_rsvp(
  text, text, integer, integer, text, text, jsonb, boolean, boolean
) to service_role;

-- 2) get_rsvp_by_token — expose a READ-ONLY consent boolean for the form
--    pre-check. CREATE OR REPLACE keeps the existing signature + ACL; the added
--    field surfaces ONLY `call_consent_at is not null` — never contact_id/phone,
--    preserving the RSVP privacy invariant (no contact identifiers leak).
...
as $function$
  select jsonb_build_object(
    'guest', jsonb_build_object(
      ...
      -- B1: read-only consent flag for the pre-check. Boolean ONLY (never the
      -- contact id/phone). NULL when the guest has no linked contact.
      'call_consent', (
        select c.call_consent_at is not null
...
-- Defensive: CREATE OR REPLACE preserves ACL, but re-assert the hardened posture.
revoke execute on function public.get_rsvp_by_token(text) from public, anon, authenticated;
