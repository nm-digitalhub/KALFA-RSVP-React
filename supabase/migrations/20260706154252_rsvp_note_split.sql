-- Split the guest note into an internal owner note and a public RSVP note.
--
-- guests.note is an owner-only CRM field, but the public RSVP surface both
-- returned it (get_rsvp_by_token) and overwrote it (submit_rsvp). This
-- migration adds guests.rsvp_note as the ONLY note field the public RSVP
-- surface may read/write, and rewires both SECURITY DEFINER functions to it.
--
-- Existing guests.note values are deliberately NOT migrated into rsvp_note:
-- authorship (owner vs guest) is unknowable, so no backfill (product decision).
--
-- rsvp_responses history is untouched: rsvp_responses.note keeps storing the
-- guest-submitted note audit trail, and the idempotency comparison against the
-- latest response row is unchanged.

begin;

alter table public.guests add column if not exists rsvp_note text;

comment on column public.guests.rsvp_note is
  'Guest-supplied RSVP note. The ONLY note field the public RSVP surface (get_rsvp_by_token / submit_rsvp) may read or write. guests.note stays owner-internal.';

comment on column public.guests.note is
  'Owner-internal CRM note. Never exposed to or written by the public RSVP surface — guest-supplied notes live in guests.rsvp_note.';

-- Byte-identical to the live definition except the guest jsonb exposes
-- rsvp_note instead of the owner-internal note.
...
-- Byte-identical to the live definition except the guests projection writes
-- rsvp_note instead of the owner-internal note. The rsvp_responses insert and
-- the idempotency comparison (_last.note = rsvp_responses.note) are unchanged.
