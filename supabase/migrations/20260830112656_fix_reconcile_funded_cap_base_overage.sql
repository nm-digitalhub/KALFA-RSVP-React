-- Fix reconcile_authorized_set's funded_cap: it predates the base+overage
-- pricing model (base_price/included_reached added 2026-07-26, two weeks
-- AFTER this RPC was written 2026-07-12) and never accounted for it. The old
-- formula `floor(auth/price)` ignores the flat base fee's included contacts
-- entirely — e.g. base=200, included=200, price=4, auth=200 (a fully-funded
-- hold with zero overage headroom) computed funded_cap=50 instead of the true
-- 200, rejecting legitimate contact additions the customer already paid for.
--
-- Fixed formula: funded_cap = min(max_contacts, included + floor(max(0, auth
-- - base) / price)). For a legacy campaign (base=0, included=0) this reduces
-- to EXACTLY the old formula — behavior-neutral for every campaign created
-- before the base+overage gate went live, matching the same base=0/included=0
-- neutrality already relied on by computeCeilingBaseOverage /
-- computeHoldAmountBaseOverage in src/lib/data/campaigns.ts.
--
-- Everything else (signature, grants, all other branches) is unchanged —
-- CREATE OR REPLACE keeps this a pure formula fix, not a rewrite.
